...
def test_conservative_reservation_calculation():
    """Verify conservative reservation calculates safe upper bounds for triage and deep."""
    ...
    prompt_triage = AnalysisPromptVersion(id=uuid.uuid4(), code="observatory_triage", version=4, stage="triage", config={"max_output_tokens": 2048})
    prompt_deep = AnalysisPromptVersion(id=uuid.uuid4(), code="observatory_deep_analysis", version=4, stage="deep_analysis", config={"max_output_tokens": 4096})

    res_triage_short = calculate_conservative_reservation("triage", entry_short, prompt_triage, 0.75, 3.75)
    res_triage_long = calculate_conservative_reservation("triage", entry_long, prompt_triage, 0.75, 3.75)
    res_deep_long = calculate_conservative_reservation("deep_analysis", entry_long, prompt_deep, 0.75, 3.75)

    assert res_triage_short >= 0.04  # Floor applied
    assert res_triage_long > res_triage_short
    assert res_deep_long > res_triage_long
