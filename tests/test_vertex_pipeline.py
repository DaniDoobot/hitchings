"""Tests for Bloque 7B: VertexAI provider, two-stage pipeline, benchmark logic.

All tests are offline — no real Vertex AI calls are made.
The google-genai SDK is mocked where needed.

Test list:
  28. test_triage_schema_valid
  29. test_deep_schema_valid
  30. test_vertex_provider_maps_triage_response
  31. test_vertex_provider_maps_deep_response
  32. test_vertex_usage_metadata_thoughts
  33. test_vertex_cost_calculation
  34. test_vertex_input_too_large
  35. test_pipeline_not_relevant_skips_deep
  36. test_pipeline_uncertain_skips_deep
  37. test_pipeline_relevant_calls_deep
  38. test_pipeline_triage_failure_no_deep
  39. test_pipeline_deep_failure_preserves_triage
  40. test_pipeline_disabled_provider
  41. test_benchmark_requires_confirm_flag
  42. test_benchmark_deterministic_sample
  43. test_benchmark_budget_hard_stop
"""
...
from app.providers.ai.vertex_ai import AnalysisInputTooLarge, VertexAIProvider
...
def make_vertex_settings(**overrides) -> Settings:
    """Build a Settings instance with Vertex AI configured for offline testing."""
    env = {
        "DATABASE_URL": "sqlite:///:memory:",
        "ANALYSIS_PROVIDER": "vertex_ai",
        "VERTEX_AI_PROJECT": "test-project-offline",
        ...
        "VERTEX_INPUT_USD_PER_MILLION_TOKENS": "0.075",
        "VERTEX_OUTPUT_USD_PER_MILLION_TOKENS": "0.30",
        **overrides,
    }
    return Settings(**env)
...
# ==============================================================================
# 30. VertexAIProvider maps triage response correctly
# ==============================================================================

@pytest.mark.asyncio
async def test_30_vertex_provider_maps_triage_response(db_session):
    """VertexAIProvider correctly maps a mocked Vertex AI triage response to AIProviderResult."""
    ...
    matrix, _ = make_matrix(db_session)
    ...
    assert result.payload.summary is None  # triage doesn't produce summary
...
# ==============================================================================
# 31. VertexAIProvider maps deep response correctly
# ==============================================================================

@pytest.mark.asyncio
async def test_31_vertex_provider_maps_deep_response(db_session):
    """VertexAIProvider correctly maps a mocked Vertex AI deep response to AIProviderResult."""
    settings = make_vertex_settings()
    provider = VertexAIProvider(settings=settings)
    ...
    deep_prompt = make_prompt(db_session, "observatory_deep_analysis", 2, "deep_analysis")
    matrix, _ = make_matrix(db_session)
    ...
    # Deep output tokens = text (200) + thoughts (150) = 350
    assert result.output_tokens == 350
    assert result.input_tokens == 2000


# ==============================================================================
# 32. Thought tokens included in total output
# ==============================================================================

@pytest.mark.asyncio
async def test_32_vertex_usage_metadata_thoughts(db_session):
    """Thought tokens from thinking_config are added to billable output tokens."""
    settings = make_vertex_settings()
    provider = VertexAIProvider(settings=settings)
    ...
    assert result.success is True
    # Total output = 50 (text) + 300 (thoughts) = 350
    assert result.output_tokens == 350
...
def test_33_vertex_cost_calculation():
    """Cost is calculated correctly from configurable pricing rates."""
    settings = make_vertex_settings(
        VERTEX_INPUT_USD_PER_MILLION_TOKENS="1.00",
        VERTEX_OUTPUT_USD_PER_MILLION_TOKENS="4.00",
    )
    provider = VertexAIProvider(settings=settings)

    # 1M input + 0.5M output
    cost = provider._calculate_cost(1_000_000, 500_000)
    expected = 1.00 + 2.00  # 1.00 input + 2.00 output
...
@pytest.mark.asyncio
async def test_34_vertex_input_too_large(db_session):
    """Provider returns failure with AnalysisInputTooLarge if content exceeds max_chars."""
    settings = make_vertex_settings(ANALYSIS_TRIAGE_MAX_INPUT_CHARS="10")  # very small limit
    provider = VertexAIProvider(settings=settings)
    provider._client = MagicMock()  # ensure client won't be needed

    source = make_source(db_session, "Test Source 34")
    entry = make_entry(db_session, source, content="A" * 20)  # 20 chars > 10 limit
    prompt = make_prompt(db_session)
    matrix, _ = make_matrix(db_session)
    from app.services.analysis_service import compute_matrix_snapshot
    snapshot, _ = compute_matrix_snapshot(matrix)
    ...
    assert "10" in result.error_message or "10" in str(result.error_message)
...
async def test_35_pipeline_not_relevant_skips_deep(db_session):
    ...
    class LowScoreProvider(MockAIProvider):
        ...
        async def analyze(self, prompt_version, entry, matrix_snapshot, **kwargs):
            ...
            # Force empty topics for low score to avoid primary topic validation
    ...
    assert call_count["analyze"] == 1  # only triage, no deep
...
async def test_37_pipeline_relevant_calls_deep(db_session):
    ...
    assert call_count["analyze"] == 2  # triage + deep
...
async def test_38_pipeline_triage_failure_no_deep(db_session):
    ...
    assert call_count["analyze"] == 1  # only triage attempted
    assert analysis.relevance_score is None

    # AnalysisCall should record the failed triage
...
async def test_39_pipeline_deep_failure_preserves_triage(db_session):
    ...
    # EntryAnalysis must be failed (deep failed)
    assert analysis.status == "failed"
    # Triage data must be preserved
    assert analysis.relevance_score == 85
    assert analysis.relevance_status == "relevant"
    assert analysis.reason is not None
    # Summary must be absent (deep failed)
    assert analysis.summary is None

    # Two calls recorded: triage (completed) + deep (failed)
    calls = sorted(
...
def test_40_pipeline_disabled_provider():
    ...
    svc = AnalysisService()  # no injected provider; reads from settings (disabled by default)
...
def test_41_benchmark_requires_confirm_flag(capsys):
    """Benchmark exits with SystemExit when real calls requested but ANALYSIS_PROVIDER != vertex_ai."""
    # Import here to avoid side effects at module level
    from scripts.run_analysis_benchmark import run_benchmark
    from app.db.session import SessionLocal

    # Use a settings with ANALYSIS_PROVIDER=disabled to simulate wrong config
    settings_patch = MagicMock()
    settings_patch.ANALYSIS_PROVIDER = "disabled"
    settings_patch.VERTEX_AI_PROJECT = ""
...
def test_42_benchmark_deterministic_sample(db_session):
    ...
    # Create 7 entries for source A (expect top 5 by date)
    entries_a = []
    for i in range(7):
        e = make_entry(
            db_session, source_a,
    ...
    # Create 3 entries for source B (expect all 3)
    entries_b = []
    for i in range(3):
        e = make_entry(
            db_session, source_b,
...
async def test_43_benchmark_budget_hard_stop(db_session):
    ...
    # We patch run_benchmark internal logic by testing the cost-stop condition directly.
    # We verify that if accumulated_cost >= max_usd, the entry is NOT processed.

    processed_entries: list[str] = []

    async def fake_run_pipeline(**kwargs):
        """Fake pipeline that costs 0.60 USD per call."""
    ...
    # Simulate the budget loop manually
    max_usd = 1.00
    accumulated_cost = 0.0
    cost_per_entry = 0.60  # exceeds budget after entry 1
    ...
    for entry in entries:
        if accumulated_cost >= max_usd:
            budget_stopped = True
            break
        result = await fake_run_pipeline(entry_id=entry.id)
        accumulated_cost += cost_per_entry
        processed_count += 1

    # After 1 entry: 0.60 accumulated. Before entry 2: 0.60 < 1.00, so entry 2 processed.
    # After 2 entries: 1.20 accumulated. Before entry 3: 1.20 >= 1.00 → stop.
