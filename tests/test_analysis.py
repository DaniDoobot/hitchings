...
def test_23_seed_analysis_prompts_idempotency(db_session: Session) -> None:
    ...
    After Bloque 7F, the seed creates 8 prompt versions:
      ...
      - observatory_triage v3, observatory_deep_analysis v3 (Gemini API grounded)
      - observatory_triage v4, observatory_deep_analysis v4 (Gemini API verbatim-first)
    """
    # Run 1
    prompts_run1 = seed_analysis_prompts(db_session)
    assert len(prompts_run1) == 14
    ...
    assert ("observatory_triage", 7) in versions1
