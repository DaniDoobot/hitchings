"""Adapter for Chillin'Competition Blog (Bloque 9B)."""
...
class ChillinCompetitionAdapter(BaseWebSourceAdapter):
    """Adapter for Chillin'Competition blog (Pablo Ibáñez Colomo / Alfonso Lamadrid)."""

    adapter_code = "chillin_competition"
    default_language = "en"
    ...
    DEFAULT_FEED_URL = "https://chillingcompetition.com/feed/"
        ...
        try:
            ...
            if resp.status_code != 200:
                raise DirectWebExtractionError(
                    f"HTTP {resp.status_code} fetching Chillin feed from {feed_url}"
                )
            ...
            if isinstance(exc, DirectWebExtractionError):
                raise
            raise DirectWebExtractionError(f"Error parsing Chillin feed: {exc}") from exc
        finally:
            if should_close:
                client.close()

    def locate_editorial_container(self, soup: BeautifulSoup) -> Optional[BeautifulSoup]:
        """Chillin'Competition places posts inside div.post."""
        return soup.find("div", class_="post") or soup.find("article") or soup.find("main")
