"""Adapter for Kluwer Competition Law Blog (Bloque 9B)."""
...
class KluwerCompetitionAdapter(BaseWebSourceAdapter):
    """Adapter for Wolters Kluwer Competition Law Blog."""

    adapter_code = "kluwer_competition"
    default_language = "en"
    ...
    DEFAULT_FEED_URL = "https://legalblogs.wolterskluwer.com/competition-blog/rss.xml"
        ...
        try:
            ...
            if resp.status_code != 200:
                raise DirectWebExtractionError(
                    f"HTTP {resp.status_code} fetching Kluwer feed from {feed_url}"
                )
            ...
            if isinstance(exc, DirectWebExtractionError):
                raise
            raise DirectWebExtractionError(f"Error parsing Kluwer feed: {exc}") from exc
        finally:
            if should_close:
                client.close()

    def locate_editorial_container(self, soup: BeautifulSoup) -> Optional[BeautifulSoup]:
        """Kluwer places article text inside div.cg3-main-article-section-cstm."""
        return (
            soup.find("div", class_="cg3-main-article-section-cstm")
            or soup.find("div", class_="cg3-article-main-section-container-cstm")
